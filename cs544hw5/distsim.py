...
def manhattan_distance(x, y):
    ...
    for i in y:
        yy += i * i
    return sum(abs(a - b) for a, b in zip(x, y))/math.sqrt(xx*yy)
...
def cossim_dense(v1,v2):
    ...
    ## TODO: delete this line and implement me
    return numpy.dot(v1,v2)/math.sqrt(numpy.dot(v1,v1) * numpy.dot(v2,v2))
    pass
...
def show_nearest(word_2_vec, w_vec, exclude_w, sim_metric):
    ...
    result = []
    ## TODO: delete this line and implement me
    for i in word_2_vec.keys():
        if i not in exclude_w:
            result.append((i,sim_metric(word_2_vec[i],w_vec)))
